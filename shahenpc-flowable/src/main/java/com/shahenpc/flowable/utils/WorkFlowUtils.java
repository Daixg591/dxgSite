package com.shahenpc.flowable.utils;
...
import com.shahenpc.flowable.common.constant.ActConstant;
import com.shahenpc.flowable.common.enums.BusinessStatusEnum;
import com.shahenpc.flowable.domain.*;
import com.shahenpc.flowable.domain.bo.TaskCompleteREQ;
import com.shahenpc.flowable.domain.vo.ActBusinessRuleVo;
import com.shahenpc.flowable.domain.vo.MultiVo;
import com.shahenpc.flowable.domain.vo.ProcessNode;
import com.shahenpc.flowable.flowable.cmd.DeleteExecutionCmd;
import com.shahenpc.flowable.flowable.cmd.DeleteTaskCmd;
import com.shahenpc.flowable.flowable.cmd.ExpressCmd;
import com.shahenpc.flowable.service.*;
...
import com.shahenpc.common.utils.JsonUtils;

import static com.shahenpc.flowable.common.constant.ActConstant.*;
...
public class WorkFlowUtils {
     ...
     * @Date: 2022/4/11 13:36
     */
    public void deleteRuntimeTask(org.flowable.task.api.Task task) {
     ...
     * @Date: 2022/5/6 19:18
     */
    public List<org.flowable.task.api.Task> createSubTask(List<org.flowable.task.api.Task> parentTaskList, String assignees) {
        List<org.flowable.task.api.Task> list = new ArrayList<>();
        for (org.flowable.task.api.Task parentTask : parentTaskList) {
        ...
        if (CollectionUtil.isNotEmpty(list) && CollectionUtil.isNotEmpty(parentTaskList)) {
            ...
            List<String> taskIds = list.stream().map(org.flowable.task.api.Task::getId).collect(Collectors.toList());
     ...
     * @Date: 2022/3/13
     */
    public TaskEntity createNewTask(org.flowable.task.api.Task currentTask, Date createTime) {
    ...
    public void sendMessage(SendMessage sendMessage, String processInstanceId) {
        ...
        List<org.flowable.task.api.Task> taskList = taskService.createTaskQuery().processInstanceId(processInstanceId).list();
        for (org.flowable.task.api.Task taskInfo : taskList) {
    ...
    public Boolean autoComplete(String processInstanceId, String businessKey, List<ActNodeAssignee> actNodeAssignees, TaskCompleteREQ req) {
        ...
        List<org.flowable.task.api.Task> list = taskService.createTaskQuery().processInstanceId(processInstanceId)
     ...
     * @Date: 2022/7/8
     */
    public void settingAssignee(org.flowable.task.api.Task task, ActNodeAssignee actNodeAssignee, Boolean multiple) {
        //按业务规则选人
     ...
     * @Date: 2021/10/21
     */
    public void setAssignee(org.flowable.task.api.Task task, List<Long> assignees) {
        if (assignees.size() == 1) {
