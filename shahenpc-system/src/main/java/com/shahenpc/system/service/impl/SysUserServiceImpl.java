...
//    }
     ...
     * @return
     */
    public List<NpcCakeDto> selectByNewLevel() {
        List<NpcCakeDto> resList = new ArrayList<>();
        SysDictData dictParam = new SysDictData();
        NewLevelVo vo = userMapper.selectByNewLevel();
        dictParam.setDictType("user_level_type");
        List<SysDictData> listDict = dictDataService.selectDictDataList(dictParam);
        for (SysDictData sysDictData : listDict) {
            switch (sysDictData.getDictValue()) {
                case "1":
                    resList.add(getNpcCakeVo(sysDictData.getDictLabel(), vo.getTown()));
                    break;
                case "2":
                    resList.add(getNpcCakeVo(sysDictData.getDictLabel(), vo.getArea()));
                    break;
                case "4":
                    resList.add(getNpcCakeVo(sysDictData.getDictLabel(), vo.getCity()));
                    break;
                case "8":
                    resList.add(getNpcCakeVo(sysDictData.getDictLabel(), vo.getProvince()));
                    break;
                case "16":
                    resList.add(getNpcCakeVo(sysDictData.getDictLabel(), vo.getCountry()));
                    break;
            }
        }
    ...
    private NpcCakeDto getNpcCakeVo(String name, int value) {
        NpcCakeDto dto = new NpcCakeDto();
        dto.setName(name);
        dto.setValue(value);
        return dto;
